<template>
  <div>
    <ul class="navBar">
      <li v-for="(item, idx) in navData" :key="idx" @click="toJump(item,idx)">
        <div class="navImg" :class="[item.class,bottomCurrent==idx?className[idx]:'']"></div>
        <p>{{ item.name }}</p>
      </li>
    </ul>
  </div>
...
export default {
  ...
  watch:{
    $route(){
      this.bottomCurrent=this.$route.query.bottomIndex;
    }
  },
  methods: {
    toJump(item, idx) {
